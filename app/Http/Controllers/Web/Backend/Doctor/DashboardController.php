...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($data) {
                    ...
                    <div class="btn-group btn-group-sm btn-group-custom" role="group" aria-label="Basic example">
                        <a href="' . route('doctor.timeslot.edit', $data->id) . '" class="btn btn-outline-primary" title="View">
        ...
        return view('backend.doctor.layouts.index');
    }
    ...
    {
        $appointments = AppointmentBooking::with('psychologist')->where('psychologist_id', Auth::user()->id)->get();
        ...
        foreach ($appointments as $appointment) {
            $events[] = [
                ...
                'appointmentDate' => $appointment->created_at->format('Y-m-d\TH:i:s'), 
            ];
        }
    
        $jsonData = json_encode($events);
    
        file_put_contents(public_path('backend/doctor/schedule.json'), $jsonData);
    
     
         return view('backend.doctor.layouts.schedule');

    }

    public function invoices()
    {
        return view('backend.doctor.layouts.invoices');
    }
    public function settings()
    {

        return view('backend.doctor.layouts.settings');
    }
